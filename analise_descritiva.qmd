...
extrair_significativas_hierarquia <- function(modelo) {
  tidy(modelo) %>%
    ...
    # Extrai o nome base da variável (ex: 'partocat' de 'partocat1')
    mutate(term_base = str_extract(term, "^[^0-9]+")) %>%
    pull(term_base) %>%
    unique()
}
...
### Passo 1: Preparação da Variável Resposta

A Regressão Beta exige que a variável resposta esteja no intervalo aberto (0, 1). Portanto, precisamos primeiro verificar os valores mínimo e máximo do nosso escore e, em seguida, transformá-lo.

```{r}
#| label: verificar-limites-pedsql
#| tbl-cap: "Verificação dos limites do escore PEDSQL Global"
summary(data$pedsqlglobal36)
```

O sumário acima nos mostra o intervalo de valores. Como a Regressão Beta não aceita os valores exatos 0 ou 1, aplicaremos uma transformação padrão para garantir que todos os dados estejam dentro do intervalo aberto (0, 1).

```{r}
#| label: transformar-pedsql-beta
# Transformação para o intervalo (0,1)
# A fórmula (y * (n - 1) + 0.5) / n é robusta para casos que contenham 0s e 1s.
n <- sum(!is.na(data$pedsqlglobal36))
data <- data %>%
  mutate(pedsqlglobal36_beta = if_else(is.na(pedsqlglobal36), NA_real_,
    (pedsqlglobal36/100 * (n - 1) + 0.5) / n)
  )

# Verificar o resultado da transformação
summary(data$pedsqlglobal36_beta)
```

### Passo 2: Ajuste do Modelo

Agora, vamos ajustar o modelo de Regressão Beta. Para uma comparação justa, usaremos o mesmo conjunto de variáveis que foi selecionado pelo nosso melhor modelo anterior, o `modelo_stepwise_refinado`.

```{r}
#| label: modelo-beta
#| tbl-cap: "Modelo de Regressão Beta para o Escore PEDSQL Global"

# Obter a fórmula do modelo stepwise refinado
formula_stepwise_refinado <- formula(modelo_stepwise_refinado)

# Substituir a variável resposta na fórmula
formula_beta <- update.formula(formula_stepwise_refinado, pedsqlglobal36_beta ~ .)
cat("Fórmula do modelo Beta:\n")
print(formula_beta)
cat("\n")

# Ajustar o modelo de regressão beta
modelo_beta <- betareg(formula_beta, data = data)
...
tbl_regression(
  ...
  as_gt() %>%
  tab_header(
    title = "Regressão Beta: Fatores Associados à Qualidade de Vida",
    subtitle = "Variável dependente: PEDSQL Global (transformado para o intervalo 0-1)"
  ) %>%
  tab_source_note(md("**Nota:** Coeficientes em escala logit. Um coeficiente positivo aumenta a média do escore, enquanto um negativo a diminui. O coeficiente (phi) é o parâmetro de precisão do modelo."))
