...
# Modelagem de Regressão Linear Múltipla

## Introdução

Nesta seção, ajustamos modelos de regressão linear múltipla para investigar quais fatores estão associados à qualidade de vida (PEDSQL Global) das crianças. O objetivo é identificar quais variáveis permanecem associadas à qualidade de vida após o ajuste conjunto, considerando diferentes blocos de variáveis (saúde, contexto social, desenvolvimento). Os resultados são apresentados de forma acessível, com foco na interpretação prática.

## Seleção das Variáveis Candidatas (p < 0.25)

```{r}
# Selecionar variáveis candidatas com p < 0.25 no teste de mediana
candidatas <- tabela_completa %>%
  filter(!is.na(p_valor_mediana) & p_valor_mediana < 0.25) %>%
  distinct(Variável) %>%
  pull(Variável)

# Mapear para nomes originais das variáveis
map_vars <- setNames(names(vars_info), unlist(vars_info))
vars_candidatas <- map_vars[candidatas]
vars_candidatas <- vars_candidatas[!is.na(vars_candidatas)]

# Visualizar variáveis candidatas
vars_candidatas
```

## Definição dos Blocos para Modelagem Hierárquica

```{r}
# Bloco 1: Saúde
bloco_saude <- intersect(vars_candidatas, c("partocat", "lme6"))

# Bloco 2: Contexto social
bloco_social <- intersect(vars_candidatas, c(
  "estadocivilmae24", "creche36",
  "tempotelacat36", "horacertacat36", "telacasa36", 
  "telalimitetempocat36", "telainteracaocat36", "telalimiteconteudocat36"
))

# Bloco 3: Desenvolvimento
bloco_desenvolvimento <- intersect(vars_candidatas, c(
  "classcogcomp36", "classmfbal36", "classmgbal36", "classmotorcomp36"
))

# Visualizar blocos
list(Saude = bloco_saude, Social = bloco_social, Desenvolvimento = bloco_desenvolvimento)
```

## Modelo Completo (Todas as Variáveis Candidatas)

```{r}
# Preparar dados para o modelo completo
modelo_vars <- vars_candidatas

data_modelo <- data %>%
  select(pedsqlglobal36, all_of(modelo_vars)) %>%
  na.omit()

# Ajustar modelo completo
modelo_completo <- lm(pedsqlglobal36 ~ ., data = data_modelo)

# Resultados do modelo completo
summary_completo <- broom::tidy(modelo_completo, conf.int = TRUE)
...
summary_completo %>%
  ...
  mutate(
    ...
    signif = ifelse(as.numeric(p.value) < 0.05, "*", "")
  ) %>%
  select(Variável = term, Coeficiente = estimate, `IC 2.5%` = conf.low, `IC 97.5%` = conf.high, `p-valor` = p.value, `Significativo (<0.05)` = signif)
```

## Modelagem Hierárquica (Stepwise Forçado por Blocos)

A seguir, ajustamos modelos em etapas, adicionando os blocos de variáveis conforme o contexto:
- **Bloco 1:** Saúde
- **Bloco 2:** Saúde + Contexto Social
- **Bloco 3:** Saúde + Contexto Social + Desenvolvimento

```{r}
# Função para ajustar modelo e apresentar resultados
ajustar_modelo <- function(vars) {
  if(length(vars) == 0) return(NULL)
  dados <- data %>% select(pedsqlglobal36, all_of(vars)) %>% na.omit()
  if(nrow(dados) < 10) return(NULL)
  mod <- lm(pedsqlglobal36 ~ ., data = dados)
...
# Bloco 1: Saúde
ajustar_modelo(bloco_saude)

# Bloco 2: Saúde + Social
ajustar_modelo(c(bloco_saude, bloco_social))

# Bloco 3: Saúde + Social + Desenvolvimento
ajustar_modelo(c(bloco_saude, bloco_social, bloco_desenvolvimento))
```

## Interpretação dos Resultados

- **Coeficiente:** Indica o quanto a variável está associada à qualidade de vida, ajustando para as demais.
- **IC 95%:** Intervalo de confiança para o coeficiente.
- **p-valor:** Indica se a associação é estatisticamente significativa (p < 0.05).
- **Significativo (<0.05):** Variáveis marcadas com * são estatisticamente significativas.

**Importante:**
- Os modelos hierárquicos mostram como a inclusão de novos blocos de variáveis pode alterar a significância das associações.
- A interpretação deve ser feita considerando o contexto clínico e social, não apenas o p-valor.

Se desejar, podemos detalhar a interpretação de cada modelo ou apresentar gráficos dos coeficientes.